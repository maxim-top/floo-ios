...
@interface BMXMulticastDelegateNode : NSObject
...
@interface BMXDelegatesContainer ()
{
...
@interface BMXMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
@implementation BMXDelegatesContainer
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	@synchronized(delegateNodes){
        BMXMulticastDelegateNode *node =
	    [[BMXMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    @synchronized(delegateNodes){
        ...
        {
            BMXMulticastDelegateNode *node = [delegateNodes objectAtIndex:(i-1)];
...
{
    @synchronized(delegateNodes){
        for (BMXMulticastDelegateNode *node in delegateNodes)
        {
            node.delegate = nil;
            #if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
            node.unsafeDelegate = nil;
...
{
	return [[BMXMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation BMXMulticastDelegateNode
...
@implementation BMXMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		BMXMulticastDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
		currentNodeIndex++;
